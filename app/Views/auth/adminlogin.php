...
<body>
    <div class="login-form">
        <h2>Welcome admin</h2>
        <form action="<?= base_url('admincheck') ?>" method="post" autocomplete="off">
            <?= csrf_field() ?>
            <?php if (!empty(session()->getFlashdata('fail'))) : ?>
                <div class="alertfailed"><?= session()->getFlashdata('fail'); ?></div>
            <?php endif ?>
            <input type="text" name="email" placeholder="admin email" value="<?= set_value('email') ?>">
            ...
            <br>
            <br>
            <a href="<?php echo site_url('adminsignup'); ?>" style="color: white">I don't have an account? Signup</a>
        </form>
    </div>
</body>
